...
from face_recognition_test import get_blink_and_liveness_status, is_live_face
from face_recognition_test import reset_liveness_state  # make sure it's imported
...
def capture_face():
    ...
    image_data = data.get("image")  # unused in Option B but preserved for compatibility
    username = session.get("username")
    mode = session.get("mode")

    if mode == "register":
        password = session.get("password")
        result = process_face_image_data(image_data, mode="register", username=username, password=password)
        session.pop("password", None)
    elif mode == "login":
        result = process_face_image_data(image_data, mode="login", username=username)
    else:
        result = {"success": False, "message": "Unknown session mode."}

    # Attach blink/liveness info
...
def video_feed():
    def generate():
        ...
        while True:
            ...
            update_latest_frame(frame.copy())  # ✅ Capture for backend use
